...
import itertools
def get_optim111(model1, model2, args):
	if args.opt == "adam":
		optimizer = optim.Adam(filter(lambda p: p.requires_grad,  itertools.chain(model1.parameters(), model2.parameters())), lr=args.lr, weight_decay=args.reg)
	elif args.opt == 'sgd':
		optimizer = optim.SGD(filter(lambda p: p.requires_grad, itertools.chain(model1.parameters(), model2.parameters())), lr=args.lr, momentum=0.9, weight_decay=args.reg)
	else:
		raise NotImplementedError
	return optimizer