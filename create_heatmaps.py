...
parser.add_argument('--feature_dir', type=str, default="tcga_lusc_feat")
parser.add_argument('--results_dir', type=str, default="heatmap_generate/luad_test")
...
if __name__ == '__main__':
	...
	# feature_extractor = convnext_base()
	...
	for i in range(len(process_stack)):
		...
		# features_path = feature_path+'/pt_files/'+slide_id+'.pt'
		# h5_path = os.path.join(feature_path+'/h5_files', slide_id+'.h5')	
		# print(features_path)
		# print(h5_path)
		...
		if data_args.process_list is not None:
			process_stack.to_csv(results_path +'/results/' + data_args.process_list.split('/')[-1], index=False)
		else:
			process_stack.to_csv(results_path+'/results/{}.csv'.format(exp_args.save_exp_code), index=False)
