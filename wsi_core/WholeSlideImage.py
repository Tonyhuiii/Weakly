...
import itertools
from wsi_core.util_classes import isInContourV1, isInContourV2, isInContourV3_Easy, isInContourV3_Hard, Contour_Checking_fn, isInContourV0
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
            ...
            if self.contours_tissue is not None and seg_display:
                if not number_contours:
                    ...
                    for cont in self.contours_tissue:
                        ...
                        cv2.rectangle(img,(int(x1*scale[0]),int(y1*scale[1])),(int(x1*scale[0])+int(w1*scale[0]),int(y1*scale[1])+int(h1*scale[1])),(255,0,0),line_thickness)
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        if isinstance(contour_fn, str):
            if contour_fn == 'four_pt':
                ...
                cont_check_fn = isInContourV0(contour=cont)
            else:
    ...
    def block_blending(self, img, vis_level, top_left, bot_right, alpha=0.5, blank_canvas=False, block_size=1024):
        ...
        for x_start in range(top_left[0], bot_right[0], block_size_x * int(round(downsample[0]))):
            for y_start in range(top_left[1], bot_right[1], block_size_y * int(round(downsample[1]))):
                # print(x_start, y_start)

                # 1. convert wsi coordinates to image coordinates via shift and scale
                x_start_img = int((x_start - shift[0]) / int(round(downsample[0])))
                y_start_img = int((y_start - shift[1]) / int(round(downsample[1])))
